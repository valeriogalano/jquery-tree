 ...
 */
$.widget("daredevel.treeajax",{_create:function(){var t=this;if(this.options.dataSourceUrl){t._lazyInit()}if(this.options.dataSourceUrl){this.element.bind("treeexpand",function(event,element){if($(element).find("ul").length){return}t._lazyLoad($(element))})}if(this.options.dataEditUrl){this.element.bind("treemove",function(event,element){t._notifyMove($(element))})}this.options.core._treeajaxInitializeNode=function(li){t._initializeNode(li)}},_initializeNode:function(li){},_lazyInit:function(){var t=this;$.ajax({url:this.options.dataSourceUrl,dataType:"json",beforeSend:function(){},complete:function(){},data:{},success:function(data){$.each(data.nodes,function(key,value){t.options.core.addNode(value)})}})},_lazyLoad:function(parentLi){var t=this;$.ajax({url:this.options.dataSourceUrl,dataType:"json",data:{node:parentLi.attr("id")},success:function(data){$.each(data.nodes,function(key,value){t.options.core.addNode(value,parentLi)})}})},_notifyMove:function(li){var t=this;var parentLi=t.options.core.parentNode(li);$.ajax({url:this.options.dataEditUrl,dataType:"json",data:{node:li.attr("id"),operation:"move",parent:parentLi.attr("id")},beforeSend:function(){alert("beforeSend")},success:function(data){alert("ok")}})},options:{dataEditUrl:"",dataSourceUrl:""}});
/*
 ...
 * @see http://tree.daredevel.com
 */
$.widget("daredevel.treecontextmenu",{_buildMenu:function(){var t=this;$("."+this.options.core.widgetBaseClass+"-contextmenu").remove();var ul=$("<ul/>",{"class":this.options.core.widgetBaseClass+"-contextmenu"});var li;$.each(this.options.contextmenuItems,function(key,value){li=t._buildMenuItem(value);ul.append(li)});ul.hide().appendTo(document.body)},_buildMenuItem:function(item){var a=$("<a/>",{html:item.html,href:item.href}).bind("click",item.onClick);var li=$("<li/>",{});return li.append(a)},_closeMenu:function(){$("."+this.options.core.widgetBaseClass+"-contextmenu").remove()},_create:function(){var t=this;this.element.find("."+this.options.core.widgetBaseClass+"-label").live("contextmenu",function(e){t.options.core.select($(this).parent("li"));t._openMenu(e);return false});this.options.core._treecontextmenuInitializeNode=function(li){t._initializeNode(li)}},_destroy:function(){},_initializeNode:function(li){},_openMenu:function(e){var t=this;this._buildMenu();$("."+this.options.core.widgetBaseClass+"-contextmenu").css({left:e.pageX,top:e.pageY}).show();$("body :not(."+this.options.core.widgetBaseClass+"-contextmenu li span)").bind("click",function(){t._closeMenu()})},options:{contextmenuItems:[{html:"move as root",href:"javascript:void(0);",onClick:function(event,element){$(this.options.core.element).tree("moveNode",$("."+this.options.core.widgetBaseClass).selected(),this.options.core.element)}},{html:"cut",href:"javascript:void(0);",onClick:function(event,element){alert("this will cut "+$("."+this.options.core.widgetBaseClass).selected())}},{html:"copy",href:"javascript:void(0);",onClick:function(event,element){alert("a")}},{html:"paste",href:"javascript:void(0);",onClick:function(event,element){alert("b")}}]}});
/*
